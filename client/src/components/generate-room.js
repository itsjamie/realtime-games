...
const styles = theme => ({
  generateRoom: {
    ...
    background: 'linear-gradient(45deg, #c66bfe 30%, #6453ff 90%)',
    border: 0,
    borderRadius: 3,
    boxShadow: '0 3px 5px 2px rgba(84, 58, 177, .3)',
    color: 'white',
    height: 48,
    padding: '0 30px',
  },
...
// pulled from https://github.com/moby/moby/blob/master/pkg/namesgenerator/names-generator.go
const adjectives = [
  "admiring",
  "adoring",
  "affectionate",
...
const GenerateRoom = props => {
  ...
  const handleOnClick = () => {
    ...
    return props.joinRoom(roomName);
  };

  return (
    <React.Fragment>
      <Button className={classes.generateRoom} onClick={() => handleOnClick()}>Generate Room</Button>
      <span className={classes.break}>OR</span>
    </React.Fragment>
  )
};

export default withStyles(styles)(GenerateRoom);